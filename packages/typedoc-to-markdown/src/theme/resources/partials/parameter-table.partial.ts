...
export const ParameterTable = (
  parameterReflections: ParameterReflection[] | TypeParameterReflection[],
  kind: 'typeParameters' | 'parameters',
) => {
  const showDefaults = hasDefaultValues(kind, parameterReflections);
  const showTypes =
    kind === 'parameters' ? true : hasTypes(parameterReflections);
  const parameters = parameterReflections as ParameterReflection[];
  ...
  const rows = parameters.map((parameter) => {
    ...
    if (showDefaults) {
      row.push(escapeString(getDefaultValue(parameter)));
    }
    if (hasComments) {
